...
above. The study period starts when the fountain was first switched on and ends when the respective AIR either
melted or broke into several ice blocks. These two dates are denoted as start and expiry dates henceforth. Each
AIR dataset was abbreviated based on the construction strategy used, prefix of the country code and the suffix
of the year of its expiry date. The construction strategies are distinguished based on whether they used
fountain scheduling strategies to regulate water supply. Those that did were codenamed automated construction
strategy whereas the rest were codenamed traditional construction strategy. All except one construction campaign
used traditional construction strategies. Therefore, traditional AIRs are referred to without explicitly
specifying their construction strategy henceforth.
...
The methodology used to acquire the radius, area and volume of AIRs (see Appendix \ref{sec:drone_method}) from
...
droplets using wind speed values and fountain characteristics. A preliminary application of this methodology is
shown in Section \ref{sec:interannual}
...
model is very sensitive to the spray radius parameter. Moreover, $r_F$ is not only determined by the fountain
...
for the Swiss experiments. Further discussion on this can be found at section \ref{}.  
...
% Field measurements are the key to successfully setting up models of AIR volume evolution. However, little field
% data is available. Moreover,   

% Comprehensive weather and water information is required both to determine the mass and energy balance of AIRs
% and to provide insight on the worldwide applicability of this water storage technology. Prior to this thesis,
% AIR volume estimates suffered from high uncertainties and were limited to the Ladakh region
% \cite{nusserSociohydrologyArtificialGlaciers2019, norphelSnowWaterHarvesting2015}. 

% This chapter describes the datasets available and provides the methodology used to estimate the ice volume
% evolution of AIRs. The equations governing the mass and energy balance of ice stupas is explained along with the
% associated datasets required for forcing, calibration and validation of this AIR model.
