...
In this chapter, we present the mass and energy balance equations that were used in these models to assess the
  ...
  italic). }      
	...
	\begin{tabular}{|llll|}
		...
		\textbf{Module}        & \textbf{Oerlemans} & \textbf{AIR} & \textbf{COSISTUPA}     \\ 
		                       & \textit{Paper III} & \textit{Paper I} & \textit{\citet{sauterCOSIPYV1Opensource2020} and Section \ref{sec:Cosistupa}}     \\ \hline
		% Objective              & Integrate glacial model & Validate modelled volume & Provide user-friendly interface  \\
  %                          & with a shape evolution module & & for model updates and application \\ 
		Shape evolution        & Simplified         & Full         & Full                     \\
		Energy balance         & Neglected temperature & 1-D temperature & 2-D temperature \\
		                       &           variation &        variation & variation \\
    Mass balance           & Neglected fountain & Neglected spatial & Full          \\
                           & water supply          &   density variations & \\ \hline
	\end{tabular}
\end{table}
...
first-order estimates of growth and decay rates under various conditions. Effects of snow cover, switching off
water during daytime, switching of water for high wind speeds, different starting dates, differences between
warm and cold winters, etc. can be evaluated.

However, validation of the model with AIR volume measurements was expected to perform poorly. This was because
the model considers the ice stupa to be a single unit with a surface temperature close to the melting point.
This assumption limits the quantification of surface processes which depend strongly on surface temperature
variations. Moreover, the model assumes unlimited water availability which is often not th case during AIR
construction. This leads to overestimation of modelled volumes. Also, the model uses a simplistic shape
evolution module. It was observed that ice stupa shape evolution depends on the fountain characteristics and
differs for the growth phase and decay phase. These shortcomings made it necessary to extend the Oerlemans model
into the AIR model described below.

\subsection{AIR model}

The objective of the AIR model was to model ice volumes  the observed integrate the data sets acquired 
